...
class _TFormFieldState extends State<TFormField> {
  ...
  TextStyle get _titleStyle {
    return row!.fieldConfig?.titleStyle ?? TextStyle(fontSize: 15, color: Colors.black87);
  }

  TextStyle get _valueStyle {
    return row!.fieldConfig?.valueStyle ?? TextStyle(fontSize: 15, color: Colors.black87);
  }

  Widget get _selectorIcon => row!.fieldConfig?.selectorIcon ?? Icon(Icons.keyboard_arrow_right);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Container(
          padding: row!.fieldConfig?.padding ?? const EdgeInsets.fromLTRB(15, 0, 15, 0),
          ...
          child: Row(
            children: [
              ...
              row!.suffixWidget != null ? row!.suffixWidget!(context, row) : SizedBox.shrink(),
            ],
          ),
        ),
              ...
              value = await (Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => TFormSelectorPage(
                    title: row!.title,
                    options: row!.options!.every((element) => (element is TFormOptionModel))
                        ? <TFormOptionModel>[...row!.options as Iterable<TFormOptionModel>]
                        : row!.options!.map((e) {
  ...
  RichText _buildRichText() {
    return RichText(
        text: TextSpan(
      text: row!.title,
      style: !_enabled ? _titleStyle.copyWith(color: _disableColor) : _titleStyle,
      children: [
        TextSpan(
          text: _require
              ? _requireStar
                  ...
                  : ""
              : "",
          style: _titleStyle.copyWith(color: _enabled ? Colors.red : _disableColor),
        )
      ],
    ));
  }
